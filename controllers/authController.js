...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    const message = `If you forgot your password,submit a PATCH request with your new password 
    and confirm to: ${resetUrl}.\nIf you didn't forget your password,please ignor this email.`
    try{
        await sendEmail({
            email: user.email,
        ...
        await user.save({validateBeforeSave: false});
        return next(new AppError('There was an error with sending email', 500));
    }
});
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    // 2) If token hasn't expired and user exist,set new password
    if(!user){
